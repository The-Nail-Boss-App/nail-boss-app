...
assert.match(studio, /const applyCurrentPolish = \(scope\) => \{[\s\S]*rememberPolishColor\(committedColor\)[\s\S]*commit\(applyBaseToSlots/, 'Polish Rack should update only from committed polish application.');
...
assert.match(studio, /<div style=\{UI\.sectionTitle\}>Polish Studio<\/div>[\s\S]*Polish Color[\s\S]*HEX[\s\S]*Polish Rack™[\s\S]*Recently Used Polish/, 'Polish Studio should contain polish color, HEX, Polish Rack, and recently used polish.');
