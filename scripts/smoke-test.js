...
function assertNailShopProfilePersistenceSource() {
  ...
  assert(blueprintGalleryRendererSource.includes("export function buildGalleryGrid") && blueprintGalleryRendererSource.includes("availableTrayArea") && blueprintGalleryRendererSource.includes("--gallery-cell-width"), "BlueprintGalleryRenderer includes a Gallery Grid Engine that sizes tray cells for enlarged cover artwork fill");
  assert(blueprintGalleryRendererSource.includes("GALLERY_ARTWORK_VIEWBOX") && blueprintGalleryRendererSource.includes("x: 18") && blueprintGalleryRendererSource.includes("y: 18") && blueprintGalleryRendererSource.includes("width: 204") && blueprintGalleryRendererSource.includes("height: 314"), "BlueprintGalleryRenderer keeps the safe outer SVG artwork bounds for no-clipping shape fidelity");
  assert(blueprintGalleryRendererSource.includes("getGalleryArtworkViewBox") && blueprintGalleryRendererSource.includes("GALLERY_DYNAMIC_VIEWBOX_LIMITS") && blueprintGalleryRendererSource.includes("artworkMagnification: [1.15, 1.2]") && blueprintGalleryRendererSource.includes('data-gallery-artwork-bounds="tight"'), "BlueprintGalleryRenderer dynamically tightens artwork bounds for the final 15-20% cover-art enlargement");
  assert(blueprintGalleryRendererSource.includes("targetCellHeightFill: [0.98, 1]") && blueprintGalleryRendererSource.includes("targetCellWidthFill: [0.92, 1]"), "BlueprintGalleryRenderer documents v1.1 artwork-first cell fill targets");
  assert(blueprintGalleryRendererSource.includes("preserveAspectRatio: 'xMidYMid meet'") && blueprintGalleryRendererSource.includes("distortionSafe: true"), "BlueprintGalleryRenderer preserves safe aspect-ratio rendering with no shape distortion");
  assert(!/transform:\s*['\"]scale\(/.test(blueprintGalleryRendererSource), "BlueprintGalleryRenderer does not rely on layout transform scale for gallery sizing");

  assert(blueprintGalleryRendererSource.includes("const trayFill = renderMode === 'gallery' ? 0.98 : 0.92") && blueprintGalleryRendererSource.includes("const nailFill = 0.99"), "Blueprint Cover artwork is enlarged for cover-first presentation");
  assert(blueprintGalleryRendererSource.includes("0.002 + (profile.maxNailLength - 0.5) * 0.006") && blueprintGalleryRendererSource.includes("%, 4px)"), "Blueprint Cover row spacing is reduced so rows read as one complete set");
  assert(blueprintGalleryRendererSource.includes("rgba(255,255,255,.18)") && blueprintGalleryRendererSource.includes("backdropFilter: 'blur(10px)'") && !blueprintGalleryRendererSource.includes("rgba(255,255,255,.48)"), "Blueprint Cover removes the inner white display board in favor of a subtle frosted tray");
  assert(blueprintGalleryRendererSource.includes('data-testid="blueprint-gallery-artwork-tray"') && blueprintGalleryRendererSource.includes("gridTemplateRows: 'repeat(2, var(--gallery-cell-height))'") && blueprintGalleryRendererSource.includes("gridTemplateColumns: 'repeat(5, var(--gallery-cell-width))'"), "BlueprintGalleryRenderer uses cell-first two rows of five nails");
  ...
  assert(nailShopSource.includes("onClick={dismissBlueprintLibraryMessage}") && nailShopSource.includes('data-testid="blueprint-save-confirmation"'), "Confirmation dismiss logic should not trigger an extra save");
  assert(nailShopSource.includes("createBlueprintLibraryRecord(blueprintToSave") && nailShopSource.includes("const blueprintToSave = activeBlueprintPreview") && nailShopSource.includes("theme: selectedBlueprintTheme") && nailShopSource.includes("collectionName: selectedBlueprintTheme.collectionLabel"), "saved blueprint includes selected content and theme");
  ...
  assert(nailShopSource.includes('blueprintCompactPreviewFrame') && nailShopSource.includes('data-testid="blueprint-library-compact-preview"'), "Blueprint Library cards use a compact preview containment frame");
  assert(nailShopSource.includes("blueprintCompactPreviewFrame") && nailShopSource.includes("maxWidth: '100%'") && nailShopSource.includes("overflow: 'hidden'"), "compact Blueprint card preview contains overflow within card width");
  assert(nailShopSource.includes('data-testid="blueprint-detail-hero-preview"') && nailShopSource.includes('<FullSetRenderer designData={selectedLibraryBlueprint.designSnapshot.fullSetData} mode="hero" />'), "Blueprint Detail hero keeps the full hero preview");
  assert(!nailShopSource.includes('data-testid="blueprint-library-card-metadata"') && !nailShopSource.includes('blueprintMetadataRow') && !nailShopSource.includes('<strong>Collection:</strong> {collectionLabel}'), "Blueprint Library cards should not repeat title or collection rows under the preview");
  assert(nailShopSource.includes('data-testid="blueprint-library-card-name"') && nailShopSource.includes('data-testid="blueprint-library-card-collection"') && nailShopSource.includes('data-testid="blueprint-library-card-status"') && nailShopSource.includes('data-testid="blueprint-library-card-readiness"'), "Blueprint Library cover cards keep blueprint name, collection, status badge, and readiness pill in the main card body");
  assert(!nailShopSource.includes('<strong>Title:</strong>') && !nailShopSource.includes('<strong>Status:</strong> {blueprint.status}'), "Blueprint Library cover cards remove admin-style Title and Status field labels");
  assert(nailShopSource.includes("overflowWrap: 'anywhere'"), "long Blueprint card text wraps safely");
  assert(nailShopSource.includes('blueprintCardActions') && nailShopSource.includes('data-testid="blueprint-library-select-button"'), "Blueprint card actions remain visible and usable");
  ...
  assert(nailShopSource.includes('data-testid="nail-recipe-viewer"') && nailShopSource.includes('📖 Nail Recipe™') && nailShopSource.includes('Production Instructions'), "Recipe Viewer exists as a first-class Blueprint Detail section");
  assert(nailShopSource.indexOf('data-testid="blueprint-detail-hero-preview"') < nailShopSource.indexOf('data-testid="nail-recipe-viewer"') && nailShopSource.indexOf('data-testid="nail-recipe-viewer"') < nailShopSource.indexOf('data-testid="blueprint-detail-pricing-guidance"'), "Recipe section renders below Hero and above Pricing");
  assert(nailShopSource.includes('data-testid="nail-recipe-steps"') && nailShopSource.includes('formatRecipeStepLabel(step)'), "Recipe steps are visible in the Recipe Viewer timeline");
  assert(nailShopSource.includes('<strong>Polish Color:</strong>') && !nailShopSource.includes('<strong>Base Color:</strong>'), "Blueprint Detail uses Polish Color wording instead of Base Color");
  assert(nailShopSource.includes('Products & Polish™') && nailShopSource.includes('Products Used™'), "Products & Polish and Products Used wording exists");
  assert(nailShopSource.includes('data-testid="recipe-technique-notes"') && nailShopSource.includes('No technique notes added.'), "Technique Notes card and empty placeholder exist");
  assert(nailShopSource.includes('data-testid="recipe-summary"') && ['Total Polish Colors:', 'Polish Types Used:', 'Techniques Used:', 'Top Coat Types:', 'Embellishments:', 'Estimated Layers:'].every((label) => nailShopSource.includes(label)), "Recipe Summary displays generated recipe totals");
  assert(nailShopSource.includes('data-testid="blueprint-detail-pricing-guidance"') && nailShopSource.includes('data-testid="blueprint-detail-creator"') && nailShopSource.includes('data-testid="blueprint-detail-theme-info"'), "Blueprint business sections remain in Detail View");
  assert(nailShopSource.includes("createRecipeFromDesign(blueprint?.designSnapshot || {})"), "Recipe Viewer derives from generated Recipe data without backend integration");
  ...
  assert(nailShopSource.includes('data-testid="blueprint-detail-design-info"') && ["Shape:", "Length:", "Width:", "Effects:", "Polish Types:", "Art Elements:", "Charms:", "Jewels:", "Decals:"].every((label) => nailShopSource.includes(label)), "Blueprint Detail design information should render");
  assert(nailShopSource.includes('data-testid="blueprint-detail-pricing-guidance"') && nailShopSource.includes("Pricing Guidance Only") && nailShopSource.includes("Not a proposal. Not a quote."), "Blueprint Detail pricing guidance should be labeled as guidance only");
  assert(nailShopSource.includes('data-testid="blueprint-detail-creator"') && nailShopSource.includes("Creator Name:") && nailShopSource.includes("Shop Name:") && nailShopSource.includes("Location:"), "Blueprint Detail creator section should render");
  assert(nailShopSource.includes('data-testid="blueprint-detail-theme-info"') && nailShopSource.includes("Typography Style:") && nailShopSource.includes("Accent Style:") && nailShopSource.includes("Collection Branding:"), "Blueprint Detail theme information should render");
  assert(nailShopSource.includes('data-testid="blueprint-detail-back-button"') && nailShopSource.includes("returnToBlueprintLibrary"), "Blueprint Detail Back to Library action should exist");
  assert(nailShopSource.includes('data-testid="blueprint-detail-duplicate-button"') && nailShopSource.includes('data-testid="blueprint-detail-rename-button"') && nailShopSource.includes('data-testid="blueprint-detail-delete-button"'), "Blueprint Detail duplicate, rename, and delete actions should exist");
  assert(nailShopSource.includes("This Blueprint is private and local-only.") && nailShopSource.includes("It is not published to Gallery, Marketplace, or Proposals."), "Blueprint Detail guardrail copy exists");
  assert(nailShopSource.includes('data-content-signature={getBlueprintContentSignature(selectedLibraryBlueprint)}'), "Blueprint Detail content signature should remain based on Blueprint content, not theme presentation");
