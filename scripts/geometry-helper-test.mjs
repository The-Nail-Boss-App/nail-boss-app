...
assert(nailCanvasSource.includes('data-zoom-containment-padding="dock-safe-expanded"') && nailCanvasSource.includes('justifyContent: "center"') && nailCanvasSource.includes('padding: HERO_CANVAS_SAFE_PADDING'), 'Hero Canvas centers the nail inside dock-safe expanded containment padding');
assert(nailCanvasSource.includes('data-default-nail-bottom-clip="prevented"') && nailCanvasSource.includes('export function heroZoomFit') && nailCanvasSource.includes('height: `min(${fit.heightVh}, calc(100% - ${HERO_CANVAS_VERTICAL_SAFE_GAP}px))`'), 'default and zoomed nails use bounded fit sizing so top and bottom are not clipped');
assert(nailCanvasSource.includes('data-zoom-containment="bounded-fit-to-container"') && nailCanvasSource.includes('data-zoom-fit-helper="heroZoomFit"') && nailCanvasSource.includes('maxWidth: "min(66%, calc((100% - 36px)))"'), 'Hero Canvas uses fit-to-container containment instead of upward-only transform zoom');
assert(nailCanvasSource.includes('const HERO_MAX_ZOOM = 1.65') && nailCanvasSource.includes('Math.min(requestedZoom, HERO_MAX_ZOOM)') && nailCanvasSource.includes('data-max-contained-zoom={HERO_MAX_ZOOM}'), 'zoom containment is capped before the Studio Dock can overlap or crop the nail');
assert(studioStylesSource.includes('minHeight: "clamp(500px, 70vh, 780px)"') && studioStylesSource.includes('paddingBottom: "clamp(22px, 4vh, 42px)"') && studioStylesSource.includes('overflow: "hidden"'), 'Hero Canvas drawing area is taller and prevents Studio Dock overlap without relying on page scroll');
...
assert(nailCanvasSource.includes('justifyContent: "center"') && nailCanvasSource.includes('export function heroZoomFit') && nailCanvasSource.includes('height: `min(${fit.heightVh}, calc(100% - ${HERO_CANVAS_VERTICAL_SAFE_GAP}px))`') && nailCanvasSource.includes('data-zoom-containment="bounded-fit-to-container"'), 'active nail canvas stays centered, zoomable, and bounded to avoid top/bottom responsive clipping');
