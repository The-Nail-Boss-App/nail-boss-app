...
assert(polishRendererSource.includes('export function PolishDefs({ nail, baseLayer, fallbackColor, uid })') && polishRendererSource.includes('const resolvedFallbackColor = fallbackColor || nail?.baseColorHex || "#E8A0BF"') && polishRendererSource.includes('resolvePolishDataForRender(baseLayer?.data || {}, resolvedFallbackColor)'), 'PolishDefs uses the same nail base color fallback as PolishSurface when no base layer exists');
