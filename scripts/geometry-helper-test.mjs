...
assert(nailCanvasSource.includes('data-zoom-containment-padding="dock-safe-expanded"') && nailCanvasSource.includes('justifyContent: "flex-start"') && nailCanvasSource.includes('padding: HERO_CANVAS_SAFE_PADDING'), 'Hero Canvas starts the full nail higher inside dock-safe expanded containment padding');
...
assert(nailCanvasSource.includes('data-zoom-containment={fit.panEnabled ? "internal-pan-at-high-zoom" : "bounded-fit-to-container"}') && nailCanvasSource.includes('data-zoom-fit-helper="heroZoomFit"') && nailCanvasSource.includes('maxWidth: "min(66%, calc((100% - 36px)))"'), 'Hero Canvas uses fit-to-container containment with internal pan at high zoom instead of label-only zoom');
...
assert(nailCanvasSource.includes('justifyContent: "flex-start"') && nailCanvasSource.includes('export function heroZoomFit') && nailCanvasSource.includes('height: fit.panEnabled ? fit.heightVh : `min(${fit.heightVh}, calc(100% - ${HERO_CANVAS_VERTICAL_SAFE_GAP}px))`') && nailCanvasSource.includes('data-zoom-containment={fit.panEnabled ? "internal-pan-at-high-zoom" : "bounded-fit-to-container"}'), 'active nail canvas starts higher, remains zoomable, and uses bounded default containment plus high-zoom internal pan');
