...
assert(nailCanvasSource.includes('justifyContent: "center"') && nailCanvasSource.includes('export function heroZoomFit') && nailCanvasSource.includes('height: fit.panEnabled ? fit.heightVh : `min(${fit.heightVh}, calc(100% - ${HERO_CANVAS_VERTICAL_SAFE_GAP}px))`') && nailCanvasSource.includes('data-zoom-containment={fit.panEnabled ? "internal-pan-at-high-zoom" : "bounded-fit-to-container"}'), 'active nail canvas stays centered, remains zoomable, and uses bounded default containment plus high-zoom internal pan');
