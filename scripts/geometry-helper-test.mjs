...
assert(nailCanvasSource.includes('data-zoom-containment-padding="dock-safe-expanded"') && nailCanvasSource.includes('justifyContent: "flex-start"') && nailCanvasSource.includes('padding: "clamp(6px, 1.4vh, 14px) 18px clamp(88px, 13vh, 132px)"'), 'Hero Canvas has sufficient vertical containment with dock-safe expanded bottom padding');
assert(nailCanvasSource.includes('data-default-nail-bottom-clip="prevented"') && nailCanvasSource.includes('const defaultNailLift = 32') && nailCanvasSource.includes('defaultNailLift + Math.max(0, visualScale - NAIL_BASELINE_SCALE) * 58'), 'default nail is lifted upward so the tip is not bottom-clipped at 100% zoom');
assert(nailCanvasSource.includes('data-zoom-transform-origin="center 34%"') && nailCanvasSource.includes('transformOrigin: "center 34%"') && nailCanvasSource.includes('translateY(-${zoomLift}px) scale(${visualScale})'), 'Hero Canvas transform origin uses a safe upper origin and upward zoom lift to preserve containment');
assert(nailCanvasSource.includes('const HERO_MAX_ZOOM = 1.48') && nailCanvasSource.includes('Math.min(Number(zoom) || 1, HERO_MAX_ZOOM)') && nailCanvasSource.includes('data-max-contained-zoom={HERO_MAX_ZOOM}'), 'zoom containment is capped before the Studio Dock can overlap the nail');
