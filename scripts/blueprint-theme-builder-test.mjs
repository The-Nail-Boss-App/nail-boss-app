...
assert(galleryRendererSource.includes('estimatedNailFootprint') && galleryRendererSource.includes('shapeFamilies') && galleryRendererSource.includes('safeTrayPadding'), 'composition profile includes shape-aware footprint, families, and padding');
assert(galleryRendererSource.includes('safeArtworkScale') && galleryRendererSource.includes('maxNailLength - 0.72'), 'long nails reduce safe artwork scale to avoid vertical clipping');
assert(galleryRendererSource.includes('maxNailWidth - 0.72') && galleryRendererSource.includes('--gallery-nail-aspect'), 'wide nails reserve horizontal room through computed aspect sizing');
...
assert(galleryRendererSource.includes("'--gallery-nail-width': 'min(calc((100% - (4 * var(--gallery-tray-gap))) / 5)") && galleryRendererSource.includes("'--gallery-nail-height': 'min(calc((100% - var(--gallery-row-gap)) / 2)"), 'primary gallery sizing uses computed width/height values');
...
assert(galleryRendererSource.includes('getBlueprintGalleryAutoScaleStyle') && galleryRendererSource.includes("--gallery-art-fill") && galleryRendererSource.includes('profile.safeArtworkScale'), 'BlueprintGalleryRenderer auto scales artwork to fill the tray');
assert(galleryRendererSource.includes('data-testid="blueprint-gallery-artwork-tray"') && galleryRendererSource.includes("gridTemplateRows: 'repeat(2, minmax(0, 1fr))'") && galleryRendererSource.includes("gridTemplateColumns: 'repeat(5, var(--gallery-nail-width))'"), 'gallery renderer uses two rows of five nails');
