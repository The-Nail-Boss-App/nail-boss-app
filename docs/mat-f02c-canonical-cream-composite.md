...
Hero Lighting remains in place in the order depth, apex, primary reflection, and edge reflection. For Cream only, depth is black and the three screen-blended illuminants are explicitly neutral white. Thus Cream lighting can change brightness and reveal form but cannot move the pigment into a pink/mauve family. Non-Cream finishes continue to use the colors supplied by `appliedLighting.reflections`, including custom lighting restored from a saved design. Test-visible metadata records the input/base pigment, optical model, stage lighting model, and overlay count.
