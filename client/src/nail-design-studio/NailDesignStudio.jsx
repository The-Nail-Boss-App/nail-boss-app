  ...
  return <g data-effect-layer="marble" data-marble-alpha="localized-stream-geometry-only" data-marble-model="primary-secondary-hairline-diffusion" data-marble-clip-authority="hero-nail-mask" clipPath={`url(#${clipId})`}>
    ...
    {streams.filter((stream) => stream.visible).map((stream) => {
      ...
      return <g key={stream.id} data-marble-stream={stream.veinClass} data-stream-id={stream.id} data-metallic={metallic || undefined}>
        ...
        {!diffusion && segments.map((path, segment) => <path key={`body-${segment}`} data-vein-component="body" d={path} stroke={marbleColor(stream.color, metallic ? -38 : 0)} strokeWidth={bodyWidth * profileWidth(stream.widthProfile, (segment + .5) / segments.length)} strokeOpacity={stream.opacity * (primary ? .34 : .28)} fill="none" strokeLinecap="round" strokeLinejoin="round" vectorEffect="non-scaling-stroke" />)}
        {segments.map((path, segment) => <path key={`core-${segment}`} data-vein-component="variable-width-core" data-width-position={segment} d={path} stroke={stream.color} strokeWidth={stream.width * profileWidth(stream.widthProfile, (segment + .5) / segments.length)} strokeOpacity={stream.opacity * finishOpacity} fill="none" strokeLinecap="round" strokeLinejoin="round" vectorEffect="non-scaling-stroke" />)}
        ...
        {stream.finish === 'Glitter' && segments.map((path, segment) => { const localWidth = stream.width * profileWidth(stream.widthProfile, (segment + .5) / segments.length); return <g key={`glitter-${segment}`}><path data-vein-component="glitter-particles" d={path} stroke={marbleColor(stream.color, 86)} strokeWidth={Math.max(.12, localWidth * .48)} strokeOpacity={stream.opacity * .92} fill="none" strokeLinecap="round" strokeDasharray=".35 3.2 .7 5.1 1 7.4" strokeDashoffset={stream.id.length + segment} vectorEffect="non-scaling-stroke"/><path data-vein-component="glitter-shadow" d={path} stroke={marbleColor(stream.color, -55)} strokeWidth={Math.max(.1, localWidth * .25)} strokeOpacity={stream.opacity * .7} fill="none" strokeLinecap="round" strokeDasharray=".3 5.8 .6 8" vectorEffect="non-scaling-stroke"/></g>; })}
...
const NailDesignStudio = forwardRef(function NailDesignStudio(_, ref) {
  ...
  const randomizeMarble = () => {
    const overrides = Object.fromEntries(Object.entries(heroDocument.nail.effect.parameters.streamOverrides || {}).map(([id, override]) => [id, Object.fromEntries(Object.entries(override).filter(([key]) => !['geometryOverride', 'widthProfile'].includes(key)))]));
  ...
  const resetVeinShape = () => {
    if (!selectedStream) return; const overrides = heroDocument.nail.effect.parameters.streamOverrides || {}; const current = { ...(overrides[selectedStream.id] || {}) };
    delete current.geometryOverride; delete current.widthProfile;
    changeFinishParameter('streamOverrides', { ...overrides, [selectedStream.id]: current });
  };
  const resetVeinAppearance = () => {
    if (!selectedStream) return; const overrides = heroDocument.nail.effect.parameters.streamOverrides || {}; const current = { ...(overrides[selectedStream.id] || {}) };
    ['color', 'formulation', 'opacity', 'softness', 'visible'].forEach((key) => delete current[key]);
    changeFinishParameter('streamOverrides', { ...overrides, [selectedStream.id]: current }); setMarbleHexDraft(heroDocument.nail.effect.parameters.veinColor);
  };
  const resetMarble = () => {
          ...
          {activeTool.id === 'technique' ? <FrenchTipControls value={frenchTips[activeNailIndex]} scope={applicationScope} onScopeChange={setApplicationScope} onChange={changeActiveFrenchTip} onApply={applyFrenchTip} notice={frenchTipNotice} /> : ['polish', 'effects'].includes(activeTool.id) ? <section className="nail-design-studio__polish-studio" aria-label={activeTool.id === 'polish' ? 'Polish Studio' : 'Effects Studio'} data-hero-material-engine="Hero Material Engine" data-hero-effect-engine="Hero Effect Engine" data-hero-lighting-engine="Hero Lighting Engine" data-hero-document-id={heroDocument.metadata.id}>
            ...
            {activeTool.id === 'effects' && heroDocument.nail.effect.id === 'Marble' && <section className="nail-design-studio__marble-controls" aria-label="Marble controls">
              ...
              {selectedStream && <fieldset><legend>Selected Vein</legend><h3>Appearance</h3><label>Color / HEX<span className="nail-design-studio__color-row"><input aria-label="Selected Vein Color" type="color" value={selectedStream.color} onChange={(event) => { const value = event.target.value.toUpperCase(); setMarbleHexDraft(value); changeStreamOverride('formulation', { ...(heroDocument.nail.effect.parameters.streamOverrides?.[selectedStream.id]?.formulation || {}), color: value, finish: selectedStream.finish }); }} /><input aria-label="Selected Vein HEX" value={marbleHexDraft} maxLength="7" onChange={(event) => setMarbleHexDraft(event.target.value.toUpperCase())} onBlur={() => /^#[0-9A-F]{6}$/.test(marbleHexDraft) ? changeStreamOverride('formulation', { ...(heroDocument.nail.effect.parameters.streamOverrides?.[selectedStream.id]?.formulation || {}), color: marbleHexDraft, finish: selectedStream.finish }) : setMarbleHexDraft(selectedStream.color)} /></span></label>
                ...
                <label><input aria-label="Selected Vein Visible" type="checkbox" checked={(heroDocument.nail.effect.parameters.streamOverrides?.[selectedStream.id]?.visible) !== false} onChange={(event) => changeStreamOverride('visible', event.target.checked)} />Visible</label><button type="button" onClick={resetVeinAppearance}>Reset Vein Appearance</button>
                <h3>Shape</h3><button type="button" aria-pressed={editMarbleShape} onClick={() => { setEditMarbleShape((active) => !active); setMoveMarble(false); }}>Edit Shape</button>
                ...
                <button type="button" onClick={resetVeinShape}>Reset Vein Shape</button></fieldset>}
            </section>}
