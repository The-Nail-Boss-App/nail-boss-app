...
import { CreativeAction, CreativeColor, CreativeDirectionSelector, CreativeModeSelector, CreativeSlider } from './StudioPrimitives';
...
const NailDesignStudio = forwardRef(function NailDesignStudio(_, ref) {
  ...
  const [ombreFirstDraft, setOmbreFirstDraft] = useState('#FF2DA0');
  const [ombreSecondDraft, setOmbreSecondDraft] = useState('#39E6F2');
  ...
  useEffect(() => setOmbreFirstDraft(activePolishColor), [activePolishColor]);
  useEffect(() => setOmbreSecondDraft(heroDocument.nail.effect.parameters.colorB || '#39E6F2'), [heroDocument.nail.effect.parameters.colorB]);
  useEffect(() => {
      ...
      <div className={`nail-design-studio__workspace${leftPanelOpen && !focusMode ? '' : ' nail-design-studio__workspace--left-closed'}${rightPanelOpen && !focusMode ? '' : ' nail-design-studio__workspace--right-closed'}`}>
        ...
        {leftPanelOpen && !focusMode && <aside id="creative-tools-panel" className="nail-design-studio__panel nail-design-studio__creative-tools" role="tabpanel" aria-labelledby={`nail-tool-${activeTool.id}`} tabIndex="0" onWheelCapture={(event) => {
          ...
          {activeTool.id === 'technique' ? <FrenchTipControls value={frenchTips[activeNailIndex]} geometry={renderedSurface} scope={applicationScope} onScopeChange={setApplicationScope} onChange={changeActiveFrenchTip} onApply={applyFrenchTip} notice={frenchTipNotice} /> : ['polish', 'effects'].includes(activeTool.id) ? <section className="nail-design-studio__polish-studio" aria-label={activeTool.id === 'polish' ? 'Polish Studio' : 'Effects Studio'} data-hero-material-engine="Hero Material Engine" data-hero-effect-engine="Hero Effect Engine" data-hero-lighting-engine="Hero Lighting Engine" data-hero-document-id={heroDocument.metadata.id}>
            ...
            {activeFinish !== 'NegativeSpace' && <><CreativeSlider label="Opacity" valueLabel={`${Math.round(appliedEffect.opacity * 100)}%`} aria-label="Opacity" min="0" max="1" step=".01" value={appliedEffect.opacity} onChange={(event) => changeFinishParameter('opacity', Number(event.target.value))} />
            {activeFinish !== 'Marble' && <><CreativeSlider label="Shine" valueLabel={`${Math.round(appliedEffect.shine * 100)}%`} aria-label="Shine" min="0" max="1" step=".01" value={appliedEffect.shine} onChange={(event) => changeFinishParameter('shine', Number(event.target.value))} />
            <CreativeSlider label="Viscosity" valueLabel={`${Math.round(appliedEffect.viscosity * 100)}%`} aria-label="Viscosity" min="0" max="1" step=".01" value={appliedEffect.viscosity} onChange={(event) => changeFinishParameter('viscosity', Number(event.target.value))} /></>}</>}
            {activeTool.id === 'effects' && heroDocument.nail.effect.id === 'Gradient' && <section className="nail-design-studio__ombre-controls" aria-label="Ombré controls">
              <header><div><small>Color · transition · light</small><h3>Ombré Blend</h3></div><span>Fluid color flow</span></header>
              <div className="nail-design-studio__ombre-result"><div className="nail-design-studio__ombre-relationship" aria-hidden="true"><span style={{ '--stop-color': activePolishColor }} /><i>Color A</i><b>→</b><i>Color B</i><span style={{ '--stop-color': heroDocument.nail.effect.parameters.colorB }} /></div><div className="nail-design-studio__ombre-preview" data-tip-orientation="down" data-direction={heroDocument.nail.effect.parameters.direction} style={{ '--ombre-a': activePolishColor, '--ombre-b': heroDocument.nail.effect.parameters.colorB, '--ombre-direction': `${heroDocument.nail.effect.parameters.direction}deg` }} role="img" aria-label={`Downward-facing nail preview blending Color A ${activePolishColor} into Color B ${heroDocument.nail.effect.parameters.colorB} at ${heroDocument.nail.effect.parameters.direction} degrees`}><i /></div><small>Nail result · tip down</small></div>
              <div className="nail-design-studio__ombre-colors">
                <CreativeColor label="First color" value={activePolishColor} hexValue={ombreFirstDraft} colorAriaLabel="Color A" hexAriaLabel="Color A HEX" onChange={(event) => changeFinishParameter(baseColorKey(heroDocument.nail.effect.id), event.target.value.toUpperCase())} onHexChange={(event) => { const next = event.target.value.toUpperCase(); if (/^#?[0-9A-F]{0,6}$/.test(next)) setOmbreFirstDraft(next); }} onHexBlur={() => /^#[0-9A-F]{6}$/.test(ombreFirstDraft) ? changeFinishParameter(baseColorKey(heroDocument.nail.effect.id), ombreFirstDraft) : setOmbreFirstDraft(activePolishColor)} />
                <CreativeColor label="Second color" value={heroDocument.nail.effect.parameters.colorB} hexValue={ombreSecondDraft} colorAriaLabel="Color B" hexAriaLabel="Color B HEX" onChange={(event) => changeFinishParameter('colorB', event.target.value.toUpperCase())} onHexChange={(event) => { const next = event.target.value.toUpperCase(); if (/^#?[0-9A-F]{0,6}$/.test(next)) setOmbreSecondDraft(next); }} onHexBlur={() => /^#[0-9A-F]{6}$/.test(ombreSecondDraft) ? changeFinishParameter('colorB', ombreSecondDraft) : setOmbreSecondDraft(heroDocument.nail.effect.parameters.colorB)} />
              </div>
              <CreativeSlider label="Blend direction" valueLabel={`${heroDocument.nail.effect.parameters.direction}°`} aria-label="Direction" min="0" max="360" value={heroDocument.nail.effect.parameters.direction} onChange={(event) => changeFinishParameter('direction', Number(event.target.value))} />
            </section>}
            {activeTool.id === 'effects' && heroDocument.nail.effect.id === 'Chrome' && <section className="nail-design-studio__effect-creative nail-design-studio__effect-creative--chrome" aria-label="Chrome controls">
              <header><small>Liquid light</small><h3>Chrome Polish</h3></header>
              <div className="nail-design-studio__chrome-preview" style={{ '--effect-color': activePolishColor, '--reflection': activeFormulation.metallicReflection ?? .76 }} role="img" aria-label={`Reflective chrome preview in ${activePolishColor}`}><i /></div>
              <CreativeColor label="Chrome color" value={activePolishColor} hexValue={activePolishColor} colorAriaLabel="Base Color picker" hexAriaLabel="Base Color HEX" onChange={(event) => changeFinishParameter('baseColor', event.target.value.toUpperCase())} />
              <CreativeSlider label="Metallic Reflection" valueLabel={`${Math.round((activeFormulation.metallicReflection ?? .76) * 100)}%`} aria-label="Metallic Reflection" min="0" max="1" step=".01" value={activeFormulation.metallicReflection ?? .76} onChange={(event) => changeFinishParameter('metallicReflection', Number(event.target.value))} />
            </section>}
            {activeTool.id === 'effects' && heroDocument.nail.effect.id === 'Cat Eye' && <section className="nail-design-studio__effect-creative nail-design-studio__effect-creative--cat-eye" aria-label="Cat Eye controls">
              <header><small>Magnetic glow</small><h3>Cat Eye Beam</h3></header>
              <div className="nail-design-studio__cat-eye-preview" style={{ '--effect-color': activePolishColor, '--stripe-angle': `${heroDocument.nail.effect.parameters.stripeDirection}deg`, '--stripe-width': `${6 + heroDocument.nail.effect.parameters.stripeWidth * 50}px`, '--stripe-strength': heroDocument.nail.effect.parameters.stripeStrength }} role="img" aria-label={`Magnetic highlight at ${heroDocument.nail.effect.parameters.stripeDirection} degrees`}><i /></div>
              <CreativeColor label="Magnetic color" value={activePolishColor} hexValue={activePolishColor} colorAriaLabel="Base Color picker" hexAriaLabel="Base Color HEX" onChange={(event) => changeFinishParameter('baseColor', event.target.value.toUpperCase())} />
              <CreativeSlider label="Stripe direction" valueLabel={`${heroDocument.nail.effect.parameters.stripeDirection}°`} aria-label="Stripe direction" min="0" max="360" value={heroDocument.nail.effect.parameters.stripeDirection} onChange={(event) => changeFinishParameter('stripeDirection', Number(event.target.value))} />
              <CreativeSlider label="Stripe width" valueLabel={`${Math.round(heroDocument.nail.effect.parameters.stripeWidth * 100)}%`} aria-label="Stripe width" min="0" max="1" step=".01" value={heroDocument.nail.effect.parameters.stripeWidth} onChange={(event) => changeFinishParameter('stripeWidth', Number(event.target.value))} />
              <CreativeSlider label="Stripe strength" valueLabel={`${Math.round(heroDocument.nail.effect.parameters.stripeStrength * 100)}%`} aria-label="Stripe strength" min="0" max="1" step=".01" value={heroDocument.nail.effect.parameters.stripeStrength} onChange={(event) => changeFinishParameter('stripeStrength', Number(event.target.value))} />
            </section>}
            {activeTool.id === 'effects' && heroDocument.nail.effect.id === 'Marble' && <section className="nail-design-studio__marble-controls" aria-label="Marble controls">
              <header className="nail-design-studio__marble-title"><small>Geological composition</small><h3>Marble Atelier</h3><p>Sculpt color, mineral finish, and natural flow.</p></header>
              <CreativeModeSelector label="Marble workspace mode" value={marbleSetMode === 'independent' ? 'independent' : 'set'} onChange={changeMarbleWorkspaceMode} options={[{ value: 'independent', label: 'Independent' }, { value: 'set', label: 'Set' }]} />
              ...
              <details className="nail-design-studio__marble-advanced"><summary>Transform</summary>
                ...
                <CreativeSlider label="Scale" valueLabel={`${Math.round((heroDocument.nail.effect.parameters.marbleTransform?.scale || 1) * 100)}%`} aria-label="Marble Scale" min=".55" max="2.5" step=".01" value={heroDocument.nail.effect.parameters.marbleTransform?.scale || 1} onChange={(event) => changeMarbleTransform('scale', Number(event.target.value))} />
                <CreativeSlider label="Rotation" valueLabel={`${heroDocument.nail.effect.parameters.marbleTransform?.rotation || 0}°`} aria-label="Marble Rotation" min="-180" max="180" value={heroDocument.nail.effect.parameters.marbleTransform?.rotation || 0} onChange={(event) => changeMarbleTransform('rotation', Number(event.target.value))} />
              </details>
              ...
              <div className="nail-design-studio__marble-streams" role="listbox" aria-label="Marble Veins">{artistMarbleStreams.map((stream) => <button type="button" role="option" aria-selected={selectedStream?.id === stream.id} data-visible={stream.visible} data-finish={stream.finish} style={{ '--vein-color': stream.color, '--vein-width': `${Math.min(8, Math.max(1.5, stream.width))}px` }} key={stream.id} onClick={() => selectMarbleStream(stream.id, stream.color)}><i aria-hidden="true"><b /></i><strong>{stream.custom ? 'Custom ' : ''}{stream.veinClass.charAt(0).toUpperCase() + stream.veinClass.slice(1)} {stream.custom ? artistMarbleStreams.filter((item) => item.custom && item.veinClass === stream.veinClass).findIndex((item) => item.id === stream.id) + 1 : Number(stream.id.split('-')[1]) + 1}</strong><small>{stream.finish} · {stream.visible ? 'On' : 'Off'}</small><span aria-hidden="true" /></button>)}</div>
              {selectedStream && <fieldset><legend>Selected Vein — {selectedStream.custom ? 'Custom ' : ''}{selectedStream.veinClass.charAt(0).toUpperCase() + selectedStream.veinClass.slice(1)}</legend><div className="nail-design-studio__selected-vein-identity" style={{ '--vein-color': selectedStream.color }}><i aria-hidden="true"><b /></i><span><strong>You’re editing this vein</strong><small>{selectedStream.veinClass} · {selectedStream.finish} · {selectedStream.width.toFixed(1)} width</small></span></div><CreativeColor label="Vein color" value={selectedStream.color} hexValue={marbleHexDraft} colorAriaLabel="Selected Vein Color" hexAriaLabel="Selected Vein HEX" onChange={(event) => { const value = event.target.value.toUpperCase(); setMarbleHexDraft(value); changeStreamOverride('formulation', { ...(heroDocument.nail.effect.parameters.streamOverrides?.[selectedStream.id]?.formulation || {}), color: value, finish: selectedStream.finish }); }} onHexChange={(event) => { const next = event.target.value.toUpperCase(); if (/^#?[0-9A-F]{0,6}$/.test(next)) setMarbleHexDraft(next); }} onHexBlur={() => /^#[0-9A-F]{6}$/.test(marbleHexDraft) ? changeStreamOverride('formulation', { ...(heroDocument.nail.effect.parameters.streamOverrides?.[selectedStream.id]?.formulation || {}), color: marbleHexDraft, finish: selectedStream.finish }) : setMarbleHexDraft(selectedStream.color)} />
                ...
                <CreativeSlider label="Opacity" valueLabel={`${Math.round(selectedStream.opacity * 100)}%`} aria-label="Selected Vein Opacity" min="0" max="1" step=".01" value={selectedStream.opacity} onChange={(event) => changeStreamOverride('opacity', Number(event.target.value))} />
                <CreativeSlider label="Softness" valueLabel={selectedStream.softness.toFixed(1)} aria-label="Selected Vein Softness" min="0" max="6" step=".1" value={selectedStream.softness} onChange={(event) => changeStreamOverride('softness', Number(event.target.value))} />
                ...
                <div className="nail-design-studio__marble-actions"><CreativeAction onClick={() => changeStreamOverride('visible', !selectedStream.visible)}>{selectedStream.visible ? 'Hide Vein' : 'Show Vein'}</CreativeAction><CreativeAction disabled={selectedStream.veinClass === 'diffusion'} onClick={duplicateMarbleVein}>Duplicate</CreativeAction><CreativeAction destructive onClick={deleteMarbleVein}>Delete</CreativeAction></div>
                <details className="nail-design-studio__marble-advanced"><summary>Accessible geometry</summary>
                  <CreativeSlider label="Overall Width" valueLabel={selectedStream.width.toFixed(1)} aria-label="Selected Vein Overall Width" min={marbleWidthBoundsForClass(selectedStream.veinClass).min} max={marbleWidthBoundsForClass(selectedStream.veinClass).max} step=".1" value={selectedStream.width} onChange={(event) => changeStreamOverride('width', Number(event.target.value))} />
                  {['start', 'middle', 'end'].map((position) => <CreativeSlider key={position} label={`${position.charAt(0).toUpperCase() + position.slice(1)} Width`} valueLabel={`${Math.round(selectedStream.widthProfile[position] * 100)}%`} aria-label={`${position.charAt(0).toUpperCase() + position.slice(1)} Width`} min=".1" max="3" step=".05" value={selectedStream.widthProfile[position]} onChange={(event) => changeStreamOverride('widthProfile', { ...selectedStream.widthProfile, [position]: Number(event.target.value) })} />)}
                </details>
            ...
            {activeTool.id === 'effects' && heroDocument.nail.effect.id === 'Aura' && <section className="nail-design-studio__effect-creative nail-design-studio__effect-creative--aura" aria-label="Aura controls">
              <header><small>Atmospheric color</small><h3>Aura Glow</h3></header>
              <div className="nail-design-studio__aura-preview" style={{ '--aura-base': activePolishColor, '--aura-center': heroDocument.nail.effect.parameters.centerColor, '--aura-color': heroDocument.nail.effect.parameters.auraColor, '--aura-softness': `${18 + heroDocument.nail.effect.parameters.softness * 34}%`, '--aura-intensity': heroDocument.nail.effect.parameters.intensity }} role="img" aria-label={`Aura preview using ${activePolishColor}, ${heroDocument.nail.effect.parameters.centerColor}, and ${heroDocument.nail.effect.parameters.auraColor}`}><i /></div>
              <div className="nail-design-studio__effect-colors nail-design-studio__effect-colors--three"><CreativeColor label="Base" value={activePolishColor} hexValue={activePolishColor} colorAriaLabel="Base Color picker" hexAriaLabel="Base Color HEX" onChange={(event) => changeFinishParameter('baseColor', event.target.value.toUpperCase())} /><CreativeColor label="Center" value={heroDocument.nail.effect.parameters.centerColor} colorAriaLabel="Aura center color" onChange={(event) => changeFinishParameter('centerColor', event.target.value.toUpperCase())} /><CreativeColor label="Halo" value={heroDocument.nail.effect.parameters.auraColor} colorAriaLabel="Aura color" onChange={(event) => changeFinishParameter('auraColor', event.target.value.toUpperCase())} /></div>
              <CreativeSlider label="Aura softness" valueLabel={`${Math.round(heroDocument.nail.effect.parameters.softness * 100)}%`} aria-label="Aura softness" min="0" max="1" step=".01" value={heroDocument.nail.effect.parameters.softness} onChange={(event) => changeFinishParameter('softness', Number(event.target.value))} />
              <CreativeSlider label="Aura intensity" valueLabel={`${Math.round(heroDocument.nail.effect.parameters.intensity * 100)}%`} aria-label="Aura intensity" min="0" max="1" step=".01" value={heroDocument.nail.effect.parameters.intensity} onChange={(event) => changeFinishParameter('intensity', Number(event.target.value))} />
            </section>}
            ...
            {activeFinish === 'Metallic' && <CreativeSlider label="Metallic Reflection" valueLabel={`${Math.round((activeFormulation.metallicReflection ?? .76) * 100)}%`} aria-label="Metallic Reflection" min="0" max="1" step=".01" value={activeFormulation.metallicReflection ?? .76} onChange={(event) => changeFinishParameter('metallicReflection', Number(event.target.value))} />}
            </section>
        ...
        {rightPanelOpen && !focusMode && <aside id="design-properties-panel" className="nail-design-studio__panel nail-design-studio__properties" aria-label="Design properties panel">
          {activeTool.id === 'effects' && heroDocument.nail.effect.id === 'Marble' && marbleSetMode !== 'independent' && <section className="nail-design-studio__marble-set" aria-label="Marble Set composition" data-marble-set-mode={marbleSetCoordination.mode} data-marble-set-seed={marbleSetCoordination.setSeed} data-marble-set-members={marbleSetCoordination.participatingNailIds.join(',')}>
            <header><span className="nail-design-studio__feature-kicker">Compose the collection</span><h2>Marble Set</h2><p>Shape one visual rhythm across the set.</p></header>
            <CreativeModeSelector label="Marble Set Style" value={marbleSetMode} onChange={changeMarbleSetStyle} options={[{ value: 'coordinated', label: 'Coordinated' }, { value: 'flow', label: 'Flow' }]} />
            <div className="nail-design-studio__marble-variation"><CreativeSlider label="Variation" valueLabel={{ low: 'Similar', medium: 'Balanced', high: 'Unique' }[marbleSetVariation]} aria-label="Marble Set Variation" aria-valuetext={{ low: 'Similar', medium: 'Balanced', high: 'Unique' }[marbleSetVariation]} type="range" min="0" max="2" step="1" value={{ low: 0, medium: 1, high: 2 }[marbleSetVariation]} onChange={(event) => setMarbleSetVariation(['low', 'medium', 'high'][Number(event.target.value)])} /><div aria-hidden="true"><span>Similar</span><i /><span>Unique</span></div></div>
            <CreativeAction className="nail-design-studio__marble-coordinate" onClick={coordinateFromThisNail}>{marbleSetCoordination.mode !== 'independent' && marbleSetCoordination.participatingNailIds.length ? 'Update Set From This Nail' : 'Coordinate From This Nail'}</CreativeAction>
            <CreativeAction className="nail-design-studio__marble-randomize" onClick={randomizeMarbleSet}>Randomize</CreativeAction>
            <details className="nail-design-studio__marble-more"><summary aria-label="More Marble Set actions">More Choices</summary><div>
              ...
              <CreativeAction onClick={applyMarbleSet}>Reapply Set Style</CreativeAction><CreativeAction destructive onClick={resetMarbleSet}>Reset Set</CreativeAction>
            </div></details>
          </section>}
