...
import {
  ...
  HeroEngineRegistry, HeroLocalStoragePersistenceAdapter, HeroSurfaceRenderingEngine, createMarbleVeinModel, marbleRibbonPath, initialHeroDesignState,
...
function MarbleVeins({ effect, nailIdentity, clipId, surfaceBounds, selectedId, editShape = false, onSelect, onPointDown }) {
  ...
  return <g data-effect-layer="marble" data-marble-alpha="localized-stream-geometry-only" data-marble-model="primary-secondary-hairline-diffusion" data-marble-clip-authority="hero-nail-mask" clipPath={`url(#${clipId})`}>
    ...
    {streams.filter((stream) => stream.visible).map((stream) => {
      ...
      return <g key={stream.id} data-marble-stream={stream.veinClass} data-stream-id={stream.id} data-metallic={metallic || undefined}>
        ...
        {diffusion ? segments.map((path, segment) => <path key={`core-${segment}`} data-vein-component="variable-width-core" data-width-position={segment} d={path} stroke={stream.color} strokeWidth={stream.width * profileWidth(stream.widthProfile, (segment + .5) / segments.length)} strokeOpacity={stream.opacity * finishOpacity} fill="none" strokeLinecap="round" strokeLinejoin="round" vectorEffect="non-scaling-stroke" />) : <g data-vein-component="localized-formulation" style={stream.softness ? { filter: `blur(${stream.softness}px)` } : undefined}><MaterialLayers path={ribbon} surfaceBounds={surfaceBounds} finish={stream.finish} color={stream.color} fleckColor={marbleColor(stream.color, 72)} glitterDensity={.46} opacity={stream.opacity} shine={effect.parameters.shine || .68} uid={`marble-${nailIdentity.replace(/[^a-z0-9]/gi, '-')}-${stream.id}`} baseProps={{ 'data-vein-component': 'variable-width-ribbon', 'data-width-profile': `${stream.widthProfile.start},${stream.widthProfile.middle},${stream.widthProfile.end}` }}/></g>}
