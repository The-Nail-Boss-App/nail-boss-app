        ...
        {leftPanelOpen && !focusMode && <aside id="creative-tools-panel" className="nail-design-studio__panel nail-design-studio__creative-tools" role="tabpanel" aria-labelledby={`nail-tool-${activeTool.id}`} tabIndex="0" onWheelCapture={(event) => {
          ...
          {activeTool.id === 'technique' ? <FrenchTipControls value={frenchTips[activeNailIndex]} scope={applicationScope} onScopeChange={setApplicationScope} onChange={changeActiveFrenchTip} onApply={applyFrenchTip} notice={frenchTipNotice} /> : ['polish', 'effects'].includes(activeTool.id) ? <section className="nail-design-studio__polish-studio" aria-label={activeTool.id === 'polish' ? 'Polish Studio' : 'Effects Studio'} data-hero-material-engine="Hero Material Engine" data-hero-effect-engine="Hero Effect Engine" data-hero-lighting-engine="Hero Lighting Engine" data-hero-document-id={heroDocument.metadata.id}>
            ...
            {activeTool.id === 'effects' && heroDocument.nail.effect.id === 'Gradient' && <section className="nail-design-studio__ombre-controls" aria-label="Ombré controls">
              <header><div><small>Color Flow</small><h3>Ombré Blend</h3></div><span>Soft transition</span></header>
              ...
              <div className="nail-design-studio__ombre-colors">
                <CreativeColor label="First color" value={activePolishColor} hexValue={activePolishColor} colorAriaLabel="Color A" hexAriaLabel="Color A HEX" onChange={(event) => changeFinishParameter(baseColorKey(heroDocument.nail.effect.id), event.target.value.toUpperCase())} />
                <CreativeColor label="Second color" value={heroDocument.nail.effect.parameters.colorB} hexValue={heroDocument.nail.effect.parameters.colorB} colorAriaLabel="Color B" hexAriaLabel="Color B HEX" onChange={(event) => changeFinishParameter('colorB', event.target.value.toUpperCase())} />
              </div>
