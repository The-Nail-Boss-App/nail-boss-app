  ...
  return <g data-effect-layer="marble" data-marble-alpha="localized-stream-geometry-only" data-marble-model="primary-secondary-hairline-diffusion" data-marble-transform={groupTransform} transform={groupTransform} clipPath={`url(#${clipId})`}>
    {streams.filter((stream) => stream.visible).map((stream) => {
      ...
      return <g key={stream.id} data-marble-stream={stream.veinClass} data-stream-id={stream.id} data-metallic={metallic || undefined}>
        ...
        {!diffusion && <path data-vein-component="body" d={stream.path} stroke={marbleColor(stream.color, metallic ? -38 : 0)} strokeWidth={bodyWidth} strokeOpacity={stream.opacity * (primary ? .34 : .28)} fill="none" strokeLinecap="round" strokeLinejoin="round" strokeDasharray={dash} vectorEffect="non-scaling-stroke" />}
        <path data-vein-component="variable-width-core" d={stream.path} stroke={stream.color} strokeWidth={stream.width} strokeOpacity={stream.opacity} fill="none" strokeLinecap="round" strokeLinejoin="round" strokeDasharray={diffusion ? '52 20 34 13' : dash} vectorEffect="non-scaling-stroke" />
        {!diffusion && <path data-vein-component="fracture" d={stream.path} stroke={marbleColor(stream.color, metallic ? 42 : 16)} strokeWidth={Math.max(.16, stream.width * (primary ? .27 : .2))} strokeOpacity={stream.opacity * .72} fill="none" strokeLinecap="round" strokeDasharray={primary ? '3 11 1 17 6 23' : '2 13 5 19'} strokeDashoffset={stream.id.length * 3} vectorEffect="non-scaling-stroke" />}
        {metallic && <path data-vein-component="metallic-highlight" d={stream.path} stroke={marbleColor(stream.color, 78)} strokeWidth={Math.max(.18, stream.width * .22)} strokeOpacity={stream.opacity * .78} fill="none" strokeLinecap="round" strokeDasharray="8 21 2 13 14 30" strokeDashoffset={stream.id.charCodeAt(0)} vectorEffect="non-scaling-stroke" />}
...
const NailDesignStudio = forwardRef(function NailDesignStudio(_, ref) {
  ...
  const randomizeMarble = () => changeFinishParameter('marbleSeed', `marble-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 9)}`);
  const resetMarble = () => {
    const next = { ...heroDocument.nail.effect.parameters, marbleTransform: { panX: 0, panY: 0, scale: 1, rotation: 0 }, streamOverrides: {} };
        ...
        {leftPanelOpen && !focusMode && <aside id="creative-tools-panel" className="nail-design-studio__panel nail-design-studio__creative-tools" role="tabpanel" aria-labelledby={`nail-tool-${activeTool.id}`} tabIndex="0" onWheelCapture={(event) => {
          ...
          {activeTool.id === 'technique' ? <FrenchTipControls value={frenchTips[activeNailIndex]} scope={applicationScope} onScopeChange={setApplicationScope} onChange={changeActiveFrenchTip} onApply={applyFrenchTip} notice={frenchTipNotice} /> : ['polish', 'effects'].includes(activeTool.id) ? <section className="nail-design-studio__polish-studio" aria-label={activeTool.id === 'polish' ? 'Polish Studio' : 'Effects Studio'} data-hero-material-engine="Hero Material Engine" data-hero-effect-engine="Hero Effect Engine" data-hero-lighting-engine="Hero Lighting Engine" data-hero-document-id={heroDocument.metadata.id}>
            ...
            {activeTool.id === 'effects' && heroDocument.nail.effect.id === 'Marble' && <section className="nail-design-studio__marble-controls" aria-label="Marble controls">
              ...
              <div className="nail-design-studio__marble-actions"><button type="button" onClick={randomizeMarble}>Randomize Layout</button><button type="button" onClick={resetMarble}>Reset Layout &amp; Veins</button></div>
              <label>Vein density <input aria-label="Vein density" type="range" min="0" max="1" step=".01" value={heroDocument.nail.effect.parameters.veinDensity} onChange={(event) => changeFinishParameter('veinDensity', Number(event.target.value))} /></label>
              <h3>Veins</h3><div className="nail-design-studio__marble-streams" role="listbox" aria-label="Marble Veins">{marbleStreams.map((stream) => <button type="button" role="option" aria-selected={selectedStream?.id === stream.id} data-visible={stream.visible} key={stream.id} onClick={() => { setSelectedMarbleStream(stream.id); setMarbleHexDraft(stream.color); }}><i style={{ backgroundColor: stream.color }} />{stream.veinClass.charAt(0).toUpperCase() + stream.veinClass.slice(1)} {Number(stream.id.split('-')[1]) + 1}<span>{stream.visible ? 'On' : 'Off'}</span></button>)}</div>
              {selectedStream && <fieldset><legend>Selected Vein</legend><label>Color / HEX<span className="nail-design-studio__color-row"><input aria-label="Selected Vein Color" type="color" value={selectedStream.color} onChange={(event) => { const value = event.target.value.toUpperCase(); setMarbleHexDraft(value); changeStreamOverride('color', value); }} /><input aria-label="Selected Vein HEX" value={marbleHexDraft} maxLength="7" onChange={(event) => setMarbleHexDraft(event.target.value.toUpperCase())} onBlur={() => /^#[0-9A-F]{6}$/.test(marbleHexDraft) ? changeStreamOverride('color', marbleHexDraft) : setMarbleHexDraft(selectedStream.color)} /></span></label>
                <label>Width <output>{selectedStream.width.toFixed(1)}</output><input aria-label="Selected Vein Width" type="range" min=".1" max={selectedStream.veinClass === 'primary' ? 8 : selectedStream.veinClass === 'hairline' ? 1.5 : 5} step=".1" value={selectedStream.width} onChange={(event) => changeStreamOverride('width', Number(event.target.value))} /></label>
                ...
                <label><input aria-label="Selected Vein Visible" type="checkbox" checked={(heroDocument.nail.effect.parameters.streamOverrides?.[selectedStream.id]?.visible) !== false} onChange={(event) => changeStreamOverride('visible', event.target.checked)} />Visible</label></fieldset>}
            </section>}
              ...
              {visibleNails.map(({ index, handClass, label }) => (
                <button type="button" className={`nail-design-studio__nail-slot nail-design-studio__nail-slot--${handClass}`} data-testid="nail-slot" data-active={activeNailIndex === index} data-selected={selectedNails.includes(index)} key={index} onClick={(event) => { selectActiveNail(index); if (event.shiftKey || event.ctrlKey || event.metaKey) setSelectedNails((items) => items.includes(index) ? items.filter((item) => item !== index) : [...items, index]); }} aria-pressed={selectedNails.includes(index)} aria-label={`Select ${label}`}>
                  ...
                  <svg className="nail-design-studio__hero-nail" data-testid="stage-nail" data-nail-shape={nailShape.toLowerCase()}
                    ...
                    <MaterialLayers path={renderedSurface.path} surfaceBounds={renderedSurface.bounds} finish={activeFinish === 'NegativeSpace' ? activeFormulation.finish : (activeNailIndex === index ? activeFinish : (nailPolishes[index]?.finish || activeFinish))} color={activeNailIndex === index ? activePolishColor : (nailPolishes[index]?.colorHex || activePolishColor)} fleckColor={activeNailIndex === index ? activeFormulation.fleckColor : (nailPolishes[index]?.fleckColor || activeFormulation.fleckColor)} glitterDensity={activeNailIndex === index ? activeFormulation.glitterDensity : (nailPolishes[index]?.glitterDensity ?? activeFormulation.glitterDensity)} opacity={activeNailIndex === index ? (appliedEffect.id === 'NegativeSpace' ? activeFormulation.opacity : appliedEffect.id === 'Marble' ? appliedEffect.opacity : appliedEffect.layers[0].opacity) : (nailPolishes[index]?.opacity ?? (appliedEffect.id === 'Marble' ? appliedEffect.opacity : appliedEffect.layers[0].opacity))} shine={activeNailIndex === index ? appliedEffect.shine : (nailPolishes[index]?.shine ?? appliedEffect.shine)} uid={`hero-material-${index}`} baseProps={{ className: 'nail-design-studio__nail-polish', 'data-design-layer': 'polish', 'data-hero-material-layer': 'true', 'data-polish-finish': activeFinish === 'NegativeSpace' ? activeFormulation.finish : (activeNailIndex === index ? activeFinish : (nailPolishes[index]?.finish || activeFinish)), 'data-hero-effect': appliedEffect.id, 'data-hero-lighting': 'Hero Lighting Engine', 'data-hero-reflection': appliedLighting.profile.reflection, 'data-material-id': renderedSurface.material.id }}/>
