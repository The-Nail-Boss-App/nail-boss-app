...
import { FINISH_DEFAULTS, VISIBLE_POLISH_FINISHES, colorBlockEffect, heroEffectForPolish, normalizePersistedAuraEffect, normalizePolishForFinish, polishSignature } from './polishFinish';
...
function initialNailDeskHeroState() {
  ...
  try {
    ...
    const document = { ...parsed, nail: { ...parsed.nail, effect: persistedEffect?.id === 'ColorBlock' ? colorBlockEffect(persistedEffect.parameters) : normalizePersistedAuraEffect(persistedEffect) || heroEffectForPolish(normalized) } };
...
const NailDesignStudio = forwardRef(function NailDesignStudio(_, ref) {
  ...
  const [selectedFinish, setSelectedFinish] = useState(() => initialNailDeskHeroState().document.nail.effect.id === 'ColorBlock' ? 'ColorBlock' : normalizePolishForFinish(loadActivePolish() || {}, loadActivePolish()?.finish || interfaceFinish(initialNailDeskHeroState().document.nail.effect.id)).finish);
  ...
  const activeFormulation = activeFinish === 'ColorBlock' ? finishFormulation : normalizePolishForFinish({ ...finishFormulation, ...heroDocument.nail.effect.parameters, name: polishName, colorHex: activePolishColor }, activeFinish);
