...
import { MaterialLayers, MaterialSwatch } from './MaterialRenderer';
...
const NailDesignStudio = forwardRef(function NailDesignStudio(_, ref) {
  ...
  const saveDesign = async () => {
    ...
    setSaveState('Saving…');
    await heroPersistence.current?.save({ ...heroDocument, metadata: { ...heroDocument.metadata, name: designName, polishFormulations: nailPolishes, activePolishFormulation: activeFormulation } });
      ...
      <div className={`nail-design-studio__workspace${leftPanelOpen && !focusMode ? '' : ' nail-design-studio__workspace--left-closed'}${rightPanelOpen && !focusMode ? '' : ' nail-design-studio__workspace--right-closed'}`}>
        ...
        {leftPanelOpen && !focusMode && <aside id="creative-tools-panel" className="nail-design-studio__panel nail-design-studio__creative-tools" role="tabpanel" aria-labelledby={`nail-tool-${activeTool.id}`} tabIndex="0">
          ...
          {['polish', 'effects'].includes(activeTool.id) ? <section className="nail-design-studio__polish-studio" aria-label={activeTool.id === 'polish' ? 'Polish Studio' : 'Effects Studio'} data-hero-material-engine="Hero Material Engine" data-hero-effect-engine="Hero Effect Engine" data-hero-lighting-engine="Hero Lighting Engine" data-hero-document-id={heroDocument.metadata.id}>
            <h3>Active Polish</h3>
            <div className="nail-design-studio__active-polish"><PolishBottle size="medium" selected colorHex={activePolishColor} polishType={activeFinish} name={polishName} collection="AnitaSet Atelier" sizeLabel="15 ml" opacity={appliedEffect.opacity} viscosity={appliedEffect.viscosity} shine={appliedEffect.shine} glitterDensity={activeFormulation.glitterDensity} shimmerIntensity={activeFormulation.shimmerIntensity} /><div className="nail-design-studio__active-details"><label>Polish name<input aria-label="Polish name" value={polishName} maxLength="48" onChange={(event) => setPolishName(event.target.value)} /></label><dl><div><dt>HEX</dt><dd>{activePolishColor}</dd></div><div><dt>Finish</dt><dd>{activeFinish}</dd></div><div><dt>Collection</dt><dd>AnitaSet Atelier</dd></div></dl><MaterialSwatch className="nail-design-studio__active-swatch" finish={activeFinish} color={activePolishColor}/></div><button type="button" className="nail-design-studio__polish-star" aria-label={activePolishSaved ? "Remove polish from Polish Rack" : "Save polish to Polish Rack"} aria-pressed={activePolishSaved} onClick={togglePolishSaved}>{activePolishSaved ? "★" : "☆"}</button></div>
            ...
            <label>Finish selection<select aria-label="Finish" value={activeFinish} onChange={(event) => changeFinish(event.target.value)}>{!VISIBLE_POLISH_FINISHES.includes(activeFinish) && activeTool.id === 'polish' && <option hidden>{activeFinish}</option>}{(activeTool.id === 'polish' ? VISIBLE_POLISH_FINISHES : Object.keys(FINISH_DEFAULTS)).map((finish) => <option key={finish}>{finish}</option>)}</select></label>
              ...
              {visibleNails.map(({ index, handClass, label }) => (
                <button type="button" className={`nail-design-studio__nail-slot nail-design-studio__nail-slot--${handClass}`} data-testid="nail-slot" data-active={activeNailIndex === index} data-selected={selectedNails.includes(index)} key={index} onClick={(event) => { selectActiveNail(index); if (event.shiftKey || event.ctrlKey || event.metaKey) setSelectedNails((items) => items.includes(index) ? items.filter((item) => item !== index) : [...items, index]); }} aria-pressed={selectedNails.includes(index)} aria-label={`Select ${label}`}>
                  ...
                  <svg className="nail-design-studio__hero-nail" data-testid="stage-nail" data-nail-shape={nailShape.toLowerCase()}
                    ...
                    <defs>{appliedEffect.layers.map((layer, layerIndex) => layer.kind === 'linear-gradient' && <linearGradient key={layerIndex} id={`hero-finish-${index}-${layerIndex}`} x1="0" y1="0" x2="1" y2="0" gradientTransform={`rotate(${layer.angle ?? 90} .5 .5)`}>{layer.colors.map((color, stop) => <stop key={stop} offset={`${stop / Math.max(1, layer.colors.length - 1) * 100}%`} stopColor={color} />)}</linearGradient>)}<radialGradient id={`hero-light-apex-${index}`} cx="50%" cy="28%" r="58%"><stop offset="0%" stopColor="#FFFFFF" stopOpacity={appliedLighting.reflections[3].opacity} /><stop offset="56%" stopColor="#FFFFFF" stopOpacity={appliedLighting.reflections[3].opacity * .22} /><stop offset="100%" stopColor="#FFFFFF" stopOpacity="0" /></radialGradient><linearGradient id={`hero-light-primary-${index}`} x1="0" y1="0" x2="1" y2="0" gradientTransform={`rotate(${appliedLighting.reflections[0].angle} .5 .5)`}><stop offset="0%" stopColor="#FFFFFF" stopOpacity="0" /><stop offset="42%" stopColor="#FFFFFF" stopOpacity={appliedLighting.reflections[0].opacity} /><stop offset="62%" stopColor="#FFFFFF" stopOpacity={appliedLighting.reflections[0].opacity * .36} /><stop offset="100%" stopColor="#FFFFFF" stopOpacity="0" /></linearGradient><linearGradient id={`hero-light-edge-${index}`} x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stopColor="#FFFFFF" stopOpacity={appliedLighting.reflections[2].opacity} /><stop offset="22%" stopColor="#FFFFFF" stopOpacity="0" /><stop offset="78%" stopColor="#FFFFFF" stopOpacity="0" /><stop offset="100%" stopColor="#FFFFFF" stopOpacity={appliedLighting.reflections[2].opacity * .72} /></linearGradient><linearGradient id={`hero-light-depth-${index}`} x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#000000" stopOpacity="0" /><stop offset="100%" stopColor="#000000" stopOpacity={appliedLighting.reflections[4].opacity} /></linearGradient></defs>
