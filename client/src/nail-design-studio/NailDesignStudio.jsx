      ...
      <div className={`nail-design-studio__workspace${leftPanelOpen && !focusMode ? '' : ' nail-design-studio__workspace--left-closed'}${rightPanelOpen && !focusMode ? '' : ' nail-design-studio__workspace--right-closed'}`}>
        ...
        {leftPanelOpen && !focusMode && <aside id="creative-tools-panel" className="nail-design-studio__panel nail-design-studio__creative-tools" role="tabpanel" aria-labelledby={`nail-tool-${activeTool.id}`} tabIndex="0">
          ...
          {['polish', 'effects'].includes(activeTool.id) ? <section className="nail-design-studio__polish-studio" aria-label={activeTool.id === 'polish' ? 'Polish Studio' : 'Effects Studio'} data-hero-material-engine="Hero Material Engine" data-hero-effect-engine="Hero Effect Engine" data-hero-lighting-engine="Hero Lighting Engine" data-hero-document-id={heroDocument.metadata.id}>
            <h3>Active Polish</h3>
            <div className="nail-design-studio__active-polish"><PolishBottle size="medium" selected colorHex={activePolishColor} polishType={activeFinish} name={polishName} collection="AnitaSet Atelier" sizeLabel="15 ml" opacity={appliedEffect.opacity} viscosity={appliedEffect.viscosity} shine={appliedEffect.shine} glitterDensity={activeFormulation.glitterDensity} shimmerIntensity={activeFormulation.shimmerIntensity} /><div className="nail-design-studio__active-details"><label>Polish name<input aria-label="Polish name" value={polishName} maxLength="48" onChange={(event) => setPolishName(event.target.value)} /></label><dl><div><dt>HEX</dt><dd>{activePolishColor}</dd></div><div><dt>Finish</dt><dd>{activeFinish}</dd></div><div><dt>Collection</dt><dd>AnitaSet Atelier</dd></div></dl><MaterialSwatch className="nail-design-studio__active-swatch" finish={activeFinish} color={activePolishColor} fleckColor={activeFormulation.fleckColor} glitterDensity={activeFormulation.glitterDensity}/></div><button type="button" className="nail-design-studio__polish-star" aria-label={activePolishSaved ? "Remove polish from Polish Rack" : "Save polish to Polish Rack"} aria-pressed={activePolishSaved} onClick={togglePolishSaved}>{activePolishSaved ? "★" : "☆"}</button></div>
