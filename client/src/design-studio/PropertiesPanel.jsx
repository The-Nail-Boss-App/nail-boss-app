...
export default function PropertiesPanel({ layer, onPatch, onDuplicate, onDelete }) {
  ...
  return (
    <section>
      ...
      {layer.type === "base" && <>
        <Color label="Base color" value={layer.data.colorHex} onChange={(colorHex) => onPatch({ data: { ...layer.data, colorHex } })} />
        <div style={UI.field}><label style={S.label}>Effect</label><select style={S.input} value={layer.data.effect} onChange={(e) => onPatch({ data: { ...layer.data, effect: e.target.value } })}>{EFFECTS.map((effect) => <option key={effect}>{effect}</option>)}</select></div>
