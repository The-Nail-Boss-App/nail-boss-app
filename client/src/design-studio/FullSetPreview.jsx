...
export default function FullSetPreview({ blueprint, activeNailId, onSelectSlot, onViewChange, hero = false }) {
  ...
  if (hero) {
    return <section data-testid="full-set-hero-canvas" data-default-view="fit-all-ten" aria-label="Full Set Mode hero canvas" style={{ height: "100%", minHeight: 0, display: "grid", gridTemplateRows: "repeat(2, minmax(0, 1fr))", gap: "clamp(12px, 2vh, 22px)", position: "relative", overflow: "hidden", background: "radial-gradient(ellipse at 50% 64%, rgba(60,20,50,.14), transparent 34%), radial-gradient(circle at 18% 16%, rgba(245,200,232,.22), transparent 28%), linear-gradient(118deg, transparent 0 17%, rgba(216,166,66,.22) 17.12%, transparent 17.48% 42%, rgba(216,166,66,.15) 42.12%, transparent 42.42% 68%, rgba(216,166,66,.16) 68.12%, transparent 68.46%), linear-gradient(135deg, #fffaf7, #fbf1ed 52%, #f7e8f1)", padding: "clamp(18px, 3vh, 30px) clamp(16px, 2.4vw, 34px)", boxSizing: "border-box" }}>
      {HERO_SET_ROWS.map((row) => (
        <div key={row.id} data-testid={`full-set-hero-${row.id}-row`} style={{ minHeight: 0, minWidth: 0, display: "grid", gridTemplateColumns: "repeat(5, minmax(0, 1fr))", alignItems: "stretch", justifyItems: "stretch", columnGap: "clamp(8px, 1.2vw, 18px)", overflow: "hidden" }}>
          {row.slots.map((slot) => {
