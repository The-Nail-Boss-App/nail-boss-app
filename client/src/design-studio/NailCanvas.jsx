...
export default function NailCanvas({ nail, layers, selectedLayerId, mode, brush, notice, debugOverlay = false, zoom = 1, onSelectLayer, onTransformLayer, onDrawingStroke, onStageEraseStroke, onEraseStroke }) {
  ...
  return <div data-testid="bounded-hero-canvas-area" data-renderer-version="nail-surface-v2" data-material-preset={activeSurfacePreset.toLowerCase()} data-zoom-containment-padding="dock-safe-expanded" data-default-nail-bottom-clip="prevented" style={{ height: "100%", minHeight: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", position: "relative", overflow: fit.panEnabled ? "auto" : "hidden", overscrollBehavior: "contain", boxSizing: "border-box", padding: HERO_CANVAS_SAFE_PADDING, background: "radial-gradient(ellipse at 50% 52%, rgba(255,255,255,.82) 0 20%, transparent 44%), radial-gradient(circle at 20% 18%, rgba(245,200,232,.30), transparent 28%), radial-gradient(circle at 84% 76%, rgba(199,154,93,.12), transparent 30%), linear-gradient(115deg, rgba(255,255,255,.34) 0 14%, transparent 14% 31%, rgba(123,47,89,.028) 31% 32%, transparent 32% 58%, rgba(255,255,255,.22) 58% 70%, transparent 70%), linear-gradient(135deg, #fffaf7, #fbf0f6 52%, #f7e8f1)" }}>
