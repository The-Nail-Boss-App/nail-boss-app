...
export default function NailCanvas({ nail, layers, selectedLayerId, mode, brush, notice, debugOverlay = false, zoom = 1, onSelectLayer, onTransformLayer, onDrawingStroke, onStageEraseStroke, onEraseStroke }) {
  ...
  return <div data-testid="bounded-hero-canvas-area" data-renderer-version="nail-surface-v2" data-material-preset={activeSurfacePreset.toLowerCase()} data-zoom-containment-padding="dock-safe-expanded" data-default-nail-bottom-clip="prevented" style={{ height: "100%", minHeight: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", position: "relative", overflow: fit.panEnabled ? "auto" : "hidden", overscrollBehavior: "contain", boxSizing: "border-box", padding: HERO_CANVAS_SAFE_PADDING, background: "radial-gradient(ellipse at 50% 52%, rgba(255,255,255,.88) 0 22%, transparent 46%), radial-gradient(circle at 28% 20%, rgba(245,200,232,.34), transparent 28%), linear-gradient(135deg, rgba(255,250,247,.96), rgba(247,232,241,.92))" }}>
    <div aria-hidden="true" data-testid="studio-surface-texture" style={{ position: "absolute", inset: "8% 7% 10%", borderRadius: 32, background: "repeating-linear-gradient(90deg, rgba(123,47,89,.045) 0 1px, transparent 1px 42px), repeating-linear-gradient(0deg, rgba(199,154,93,.04) 0 1px, transparent 1px 42px), linear-gradient(135deg, rgba(255,255,255,.54), rgba(245,200,232,.22))", border: "1px solid rgba(123,47,89,.10)", boxShadow: "inset 0 1px 0 rgba(255,255,255,.72), 0 24px 70px rgba(60,20,50,.10)", pointerEvents: "none" }} />
