  ...
  return <div style={UI.shell}>
    ...
    <div style={UI.layout}>
      ...
      <aside style={UI.panel}><div style={UI.panelPad}>
        ...
        {tab === "assets" && <><AssetLibrary onAddAsset={addAsset}/><section style={{ border: `1px solid ${COLORS.border}`, borderRadius: 16, padding: 12, marginBottom: 18, background: "#fff" }}><div style={UI.sectionTitle}>Layer Effects</div><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><button type="button" onClick={addGradient} style={{ ...S.btnSecondary, padding: "9px 12px" }}>Add Gradient</button><button type="button" onClick={addPattern} style={{ ...S.btnSecondary, padding: "9px 12px" }}>Add Pattern</button></div><p style={UI.smallText}>Add reusable gradient and pattern overlays as editable layers while the base polish surface stays flat.</p></section><DrawingToolbar brush={brush} mode={mode} onBrushChange={(patch) => setBrush((prev) => ({ ...prev, ...patch }))}/></>}
