...
export const COMPOSITION_PRESETS = [
  { id: "all-same", name: "All Same", slots: FULL_SET_SLOTS },
  { id: "accent-ring", name: "Accent Ring Finger", slots: ["left-ring", "right-ring"] },
  { id: "accent-middle", name: "Accent Middle Finger", slots: ["left-middle", "right-middle"] },
  { id: "accent-ring-thumb", name: "Accent Ring + Thumb", slots: ["left-thumb", "left-ring", "right-thumb", "right-ring"] },
  { id: "french-set", name: "French Set", slots: FULL_SET_SLOTS, templateId: "classic-french-set" },
  { id: "alternating-set", name: "Alternating Set", slots: FULL_SET_SLOTS.filter((_slot, index) => index % 2 === 0) },
  { id: "every-other-nail", name: "Every Other Nail", slots: FULL_SET_SLOTS.filter((_slot, index) => index % 2 === 1) },
  { id: "custom-selection", name: "Custom Selection", slots: [] },
];

function templateNailLayers(slot, patch, layerFactory = () => []) {
  const nail = defaultNailForSlot(slot, patch);
  return signatureLookNail(slot, patch, layerFactory(nail));
}

function buildTemplate(id, name, patch, accentSlots = [], accentLayerFactory = () => [], baseLayerFactory = () => []) {
  return {
    id,
    name,
    nails: FULL_SET_SLOTS.map((slot) => templateNailLayers(slot, patch, accentSlots.includes(slot) ? accentLayerFactory : baseLayerFactory)),
  };
}

export const SET_TEMPLATES = [
  buildTemplate("classic-french-set", "Classic French Set", { shape: "Square", length: 0.54, width: 0.5, baseColorHex: "#F7D7E6", baseData: { polishType: "Cream" } }, FULL_SET_SLOTS, (nail) => [signatureFrench(nail, { style: "classic", preset: "medium", colorHex: "#FFFFFF" })]),
  buildTemplate("bridal-set", "Bridal Set", { shape: "Almond", length: 0.62, width: 0.48, baseColorHex: "#F8DDE8", baseData: { polishType: "Milky", transparency: 0.22 } }, ["left-ring", "right-ring"], (nail) => [signatureFrench(nail, { style: "classic", preset: "soft", colorHex: "#FFFFFF" }), signatureAsset(nail, "jewel", "Pearl crystal accent", { assetId: "jewel-round", colorHex: "#FFFFFF" }, { x: 0.5, y: 0.72, scaleX: 0.14, scaleY: 0.14, rotation: 0 })], (nail) => [signatureFrench(nail, { style: "classic", preset: "soft", colorHex: "#FFFFFF" })]),
  buildTemplate("minimal-nude-set", "Minimal Nude Set", { shape: "Oval", length: 0.5, width: 0.5, baseColorHex: "#E8C7B0", baseData: { polishType: "Cream" } }, ["left-ring", "right-ring"], (nail) => [signatureAsset(nail, "decal", "Minimal gold line", { assetId: "gold-line", colorHex: "#D4AF37" }, { x: 0.5, y: 0.52, scaleX: 0.07, scaleY: 0.38, rotation: 0 }, 0.85)]),
  buildTemplate("zebra-accent-set", "Zebra Accent Set", { shape: "Coffin", length: 0.66, width: 0.52, baseColorHex: "#F5D3E1", baseData: { polishType: "Cream" } }, ["left-ring", "right-ring"], (nail) => [signaturePattern(nail, "zebra", { colorHex: "#FFFFFF", secondaryColorHex: "#111111", density: 0.74 }, 0.9)]),
  buildTemplate("leopard-accent-set", "Leopard Accent Set", { shape: "Almond", length: 0.6, width: 0.48, baseColorHex: "#E7B88F", baseData: { polishType: "Cream" } }, ["left-ring", "right-ring"], (nail) => [signaturePattern(nail, "leopard", { colorHex: "#6B3B1A", secondaryColorHex: "#1B120D", density: 0.72 }, 0.86)]),
  buildTemplate("glam-crystal-accent-set", "Glam Crystal Accent Set", { shape: "Stiletto", length: 0.72, width: 0.45, baseColorHex: "#F2D7E6", baseData: { polishType: "Jelly", transparency: 0.25 } }, ["left-ring", "right-ring"], (nail) => [signatureGradient(nail, { direction: "aura", colorA: "#FFFFFF", colorB: "#E8A0BF", softness: 0.8 }, 0.52), signatureAsset(nail, "jewel", "Crystal accent", { assetId: "jewel-round", colorHex: "#DDF7FF" }, { x: 0.5, y: 0.62, scaleX: 0.2, scaleY: 0.2, rotation: 0 })]),
];

export function slotsForCompositionPreset(presetId, customSlots = []) {
  if (presetId === "custom-selection") return customSlots.filter((slot) => FULL_SET_SLOTS.includes(slot));
  return [...(COMPOSITION_PRESETS.find((preset) => preset.id === presetId)?.slots || [])];
}

export function applyNailDesignToSlots(blueprint, sourceSlot, destinationSlots = []) {
  return copyNailToSlots(ensureFullSetBlueprint(blueprint), sourceSlot, destinationSlots);
}

export function applySignatureLookToSlots(blueprint, look, destinationSlots = []) {
  const normalizedLook = normalizeSignatureLook(look);
  const current = ensureFullSetBlueprint(blueprint);
  const targets = new Set(destinationSlots.filter((slot) => FULL_SET_SLOTS.includes(slot)));
  return { ...current, nails: current.nails.map((nail) => targets.has(nail.slot) ? cloneNailDesign(normalizedLook.scope === "full-set" ? (normalizedLook.nails.find((source) => source.slot === nail.slot) || normalizedLook.nails[0]) : normalizedLook.nail, nail) : nail) };
}

export function mirrorCurrentHand(blueprint) {
  const active = getActiveNail(ensureFullSetBlueprint(blueprint));
  return mirrorHandDesign(blueprint, active.slot?.startsWith("left") ? "left" : "right");
}

export function mirrorFullSet(blueprint) {
  return mirrorHandDesign(mirrorHandDesign(ensureFullSetBlueprint(blueprint), "left"), "right");
}

export function applySetTemplate(blueprint, templateId) {
  const template = SET_TEMPLATES.find((item) => item.id === templateId) || SET_TEMPLATES[0];
  const current = ensureFullSetBlueprint(blueprint);
  const bySlot = new Map(template.nails.map((nail) => [nail.slot, nail]));
  return { ...current, nails: current.nails.map((nail) => cloneNailDesign(bySlot.get(nail.slot) || template.nails[0], nail)), metadata: { ...current.metadata, setTemplateId: template.id, setTemplateName: template.name, templateEditable: true } };
}
