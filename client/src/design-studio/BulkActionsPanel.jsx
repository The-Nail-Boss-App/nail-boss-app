...
export default function BulkActionsPanel({ activeSlot, clipboard, selectedSlots, onToggleSlot, onCopy, onPaste, onDuplicate, onMirror, onApplyBase, onApplyShape, onReset }) {
  ...
  return <div style={{ borderTop: "1px solid #F1D7E6", marginTop: 12, paddingTop: 12 }}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
      <button type="button" style={S.btnSecondary} onClick={onCopy}>Copy active nail</button>
      <button type="button" style={S.btnSecondary} disabled={!clipboard} onClick={onPaste}>Paste to selected</button>
    ...
    <div style={{ marginTop: 10, display: "flex", flexWrap: "wrap", gap: 6 }}>{FULL_SET_SLOTS.map((slot) => <label key={slot} style={{ fontSize: 11 }}><input type="checkbox" checked={selectedSlots.includes(slot)} onChange={() => onToggleSlot(slot)}/> {slotLabel(slot)}</label>)}</div>
  </div>;
}
