...
export default function NailThumbnail({ nail, active = false, onClick, hero = false }) {
  ...
  return <button type="button" onClick={onClick} aria-pressed={active} aria-label={`Edit ${slotLabel(nail.slot)} nail`} data-full-set-hero-nail={hero ? "true" : undefined} style={hero ? { border: active ? `2px solid ${COLORS.softGold}` : "2px solid transparent", background: "transparent", borderRadius: 999, padding: "4px", minWidth: 0, minHeight: 0, width: "100%", height: "100%", maxHeight: "100%", overflow: "hidden", display: "grid", placeItems: "center", cursor: "pointer", filter: active ? "drop-shadow(0 0 10px rgba(216,166,66,.42))" : "drop-shadow(0 10px 10px rgba(60,20,50,.14))", boxSizing: "border-box" } : { border: `2px solid ${active ? COLORS.plum : COLORS.border}`, background: active ? COLORS.roseDim : "#fff", borderRadius: 16, padding: 8, minWidth: 86, cursor: "pointer", boxShadow: active ? "0 10px 24px rgba(90,44,80,.18)" : "none" }}>
    <svg viewBox={`0 0 ${VIEWBOX.width} ${VIEWBOX.height}`} width={hero ? "100%" : "70"} height={hero ? "100%" : "104"} role="img" aria-label={`${slotLabel(nail.slot)} preview`} style={{ display: "block", margin: "0 auto", maxHeight: hero ? "100%" : undefined, maxWidth: hero ? "100%" : undefined }}>
