...
- The eraser workflow removes the nearest stroke in a visible unlocked drawing layer rather than doing partial path boolean erasure. Hidden or locked drawing layers are ignored and left unchanged; if no visible unlocked drawing layer exists, the studio shows a non-blocking notice and does not create a layer merely for erasing.
